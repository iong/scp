program ljmc
        ...
        real*8, parameter :: mass = 2.0, taui = 1.0e-6, bl=40.0, rtol=1e-4, atol=1.0e-4, rc=10.0
...
!        write(*,*) 'Seed size =', seed_size
        ...
        !call  vgwinit(natom, mass/48.5086, 4, LJC, LJA, bl, rc, taui, atol, rtol)
        call  vgwinit(natom, mass*0.020614788876D0, 4, LJC, LJA, bl, rc, taui, atol, rtol)
        q00 = q0
       call cpu_time(et(1))
!$OMP PARALLEL SHARED(Y, UPV_SHARED, UPM_SHARED) PRIVATE(tid,nthreads) FIRSTPRIVATE(NEQ,ISTATE,T)
        do i=1,ntests
                ...
                !q0 = q00 + dq0*0.1
                call vgw1(q0,Ueff(i, 1), TAUMAX,0.0, y0)
        enddo
!$OMP END PARALLEL
        call cpu_time(et(2))
!$POMP INST BEGIN(LJMCSTOP)
        ...
        do i=1,ntests
                call random_number(dq0)
                !q0 = q00 + dq0*0.1
                call vgwquenchspb(natom,mass,q0,FX,LNP,Ueff(i,2), ENRG,TAUMAX,TAUI,BL,ATOL,RC,Y, InvMeff,SqrtMeff)
        enddo
        call cpu_time(et(4))
